...
if [[ ! -d "${FRONTEND_DIST}" ]]; then
  ...
  echo "Run: cd frontend_dev && npm run build" >&2
  exit 1
fi

cd "${REPO_DIR}"

PYTHONPATH="${REPO_DIR}/backend" /usr/bin/env python3 -m gunicorn -w 1 -k uvicorn.workers.UvicornWorker main:app -b 127.0.0.1:9001 &
backend_pid=$!
