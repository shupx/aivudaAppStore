...
- HTTPS `8580` 仅反代 `/aivuda_app_store/store*` 到后端公开商店接口，并把其余路径作为 `backend/data/files/apps` 文件服务
...
- HTTPS: `https://<host>:8580`（仅 `/aivuda_app_store/store*` 走后端，其他路径走 `backend/data/files/apps` 文件服务）
