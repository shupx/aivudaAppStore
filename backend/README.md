...
- HTTPS `:8580`：`/aivuda_app_store/store*` 反代到后端公开接口；`/aivuda_app_store/files/*` 由 Caddy 静态托管
- HTTPS `:8543`：保留原有 appstore_site 路由（前端静态托管 + `/aivuda_app_store*` 反代），并支持 `/aivuda_app_store/files/*` 静态下载
