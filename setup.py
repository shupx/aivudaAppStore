...
SDIST_EXCLUDED_PATHS = (
    ...
    Path("aivudaappstore/resources/ui/.vite"),
    Path("aivudaappstore/resources/ui/.vite-temp"),
)
...
def build_version() -> str:
    build_date = os.environ.get("AIVUDAAPPSTORE_BUILD_DATE", "").strip()
    if not build_date:
        build_date = datetime.now().strftime("%Y%m%d")

    build_seq = os.environ.get("AIVUDAAPPSTORE_BUILD_SEQ", "01").strip() or "01"
    if not build_date.isdigit() or len(build_date) != 8:
        raise RuntimeError("Invalid AIVUDAAPPSTORE_BUILD_DATE: {0!r}".format(build_date))
    if not build_seq.isdigit():
        raise RuntimeError("Invalid AIVUDAAPPSTORE_BUILD_SEQ: {0!r}".format(build_seq))

    return "{0}.dev{1}{2}".format(BASE_VERSION, build_date, build_seq.zfill(2))
...
def ensure_ui_dist() -> None:
    ...
    if not package_json.exists():
        if dist_dir.exists():
            return
        raise RuntimeError("UI package.json not found: {0}".format(package_json))

    npm_cmd = shutil.which("npm")
    if npm_cmd is None:
        raise RuntimeError("npm is required to build aivudaappstore/resources/ui/dist before packaging")
...
class build_py(_build_py):
    ...
    def _copy_packaged_resources(self) -> None:
        source_path = SOURCE_ROOT / "aivudaappstore" / "resources"
        destination_path = Path(self.build_lib) / "aivudaappstore" / "resources"
        if destination_path.exists():
            shutil.rmtree(destination_path)
        shutil.copytree(source_path, destination_path, ignore=self._ignore_packaged_resources)
...
class sdist(_sdist):
    ...
    def make_release_tree(self, base_dir: str, files: list) -> None:
        filtered_files = [file_path for file_path in files if not _should_exclude_from_sdist(Path(file_path))]
...
setup(
    ...
    description="Aivuda AppStore packaged for pip installation and PyPI publishing.",
